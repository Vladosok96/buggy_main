...
void BLDCMotor::init() {
  if (!driver || !driver->initialized) {
    ...
    ESP_LOGI(TAG, "MOT: Init not possible, driver not initialized");
    return;
  }
  motor_status = FOCMotorStatus::motor_initializing;
  ESP_LOGI(TAG, "MOT: Init");
  ...
  // enable motor
  ESP_LOGI(TAG, "MOT: Enable driver.");
  enable();
  _delay(100);
  ...
  if(exit_flag){
    if(current_sense){ 
      ...
      if (!current_sense->initialized) {
        ...
        ESP_LOGE(TAG, "MOT: Init FOC error, current sense not initialized");
        exit_flag = 0;
      }else{
        exit_flag *= alignCurrentSense();
      }
    }
    else ESP_LOGI(TAG, "MOT: No current sense.");
  }

  if(exit_flag){
    ESP_LOGI(TAG, "MOT: Ready.");
    motor_status = FOCMotorStatus::motor_ready;
  }else{
    ESP_LOGE(TAG, "MOT: Init FOC failed.");
    motor_status = FOCMotorStatus::motor_calib_failed;
    disable();
  }
...
int BLDCMotor::alignCurrentSense() {
  ...
  ESP_LOGI(TAG, "MOT: Align current sense.");

  // align current sense and the driver
  exit_flag = current_sense->driverAlign(voltage_sensor_align);
  if(!exit_flag){
    // error in current sense - phase either not measured or bad connection
    ESP_LOGI(TAG, "MOT: Align error!");
    exit_flag = 0;
  }else{
    // output the alignment status flag
    ESP_LOGI(TAG, "MOT: Success: %d", exit_flag);
  }
...
int BLDCMotor::alignSensor() {
  ...
  if(!_isset(sensor_direction)){
    ...
    if (moved<MIN_ANGLE_DETECT_MOVEMENT) { // minimum angle to detect movement
      ESP_LOGE(TAG, "MOT: Failed to notice movement");
      return 0; // failed calibration
    } else if (mid_angle < end_angle) {
      ESP_LOGI(TAG, "MOT: sensor_direction==CCW");
      sensor_direction = Direction::CCW;
    } else{
      ESP_LOGI(TAG, "MOT: sensor_direction==CW");
      sensor_direction = Direction::CW;
    }
    // check pole pair number
    if( fabs(moved*pole_pairs - _2PI) > 0.5f ) { // 0.5f is arbitrary number it can be lower or higher!
      ESP_LOGE(TAG, "MOT: PP check: fail - estimated pp: %f", _2PI/moved);
    } else 
      ESP_LOGI(TAG, "MOT: PP check: OK!");

  } else ESP_LOGI(TAG, "MOT: Skip dir calib.");
  ...
  if(!_isset(zero_electric_angle)){
    ...
    _delay(20);
    ESP_LOGI(TAG, "MOT: Zero elec. angle: %f", zero_electric_angle);
    // stop everything
    setPhaseVoltage(0, 0, 0);
    _delay(200);
  }else ESP_LOGI(TAG, "MOT: Skip offset calib.");
  return exit_flag;
}
...
int BLDCMotor::absoluteZeroSearch() {
  ...
  if(sensor->needsSearch()) ESP_LOGI(TAG, "MOT: Error: Not found!");
  else ESP_LOGI(TAG, "MOT: Success!");
...
void BLDCMotor::loopFOC() {
  ...
  switch (torque_controller) {
    ...
    default:
      ...
      ESP_LOGI(TAG, "MOT: no torque control selected!");
      break;
  }
...
void BLDCMotor::setPostions(float p_value, float r_value, float n_value, float d_value, float threshold_value) {
  position_p = p_value;
  position_r = r_value;
  position_n = n_value;
  position_d = d_value;
